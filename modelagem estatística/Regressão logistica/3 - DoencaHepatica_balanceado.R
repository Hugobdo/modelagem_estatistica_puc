...
# -------------------------------
# Retirando a variável ALANINA             
modelo4 <- glm(DOENCA_HEPATICA ~ . -SEXO -FOSFOTASE_ALCALINA -ALANINA, 
               data = dados_treino_balanceado, family = "binomial"(link = "logit"))
summary(modelo4)

modelo5 <- glm(DOENCA_HEPATICA ~ . -SEXO -FOSFOTASE_ALCALINA -ALANINA -ALBUMINA_GLOBULINA, 
               data = dados_treino_balanceado, family = "binomial"(link = "logit"))
summary(modelo5)
...
# Multicolinearidade
library(car)
vif(modelo4) # valores abaixo de 5 - OK
...
# verificamos que ela é significativa para o modelo. vamos tentar retirar a ALBUMINA_GLOBULINA
...
# verificamos que ela é significativa para o modelo. vamos tentar retirar a PROTEINAS_TOTAIS
# Retirando a variável PROTEINAS_TOTAIS
modelo5 <- glm(DOENCA_HEPATICA ~ . -SEXO -FOSFOTASE_ALCALINA -ALANINA -PROTEINAS_TOTAIS, 
               data = dados_treino_balanceado, family = "binomial"(link = "logit"))
summary(modelo5)
...
# todas são significativas. Vamos retirar a ALBUMINA que é correlacionada com as outras duas
# Retirando a variável ALBUMINA
modelo5 <- glm(DOENCA_HEPATICA ~ . -SEXO -FOSFOTASE_ALCALINA -ALANINA -ALBUMINA, 
               data = dados_treino_balanceado, family = "binomial"(link = "logit"))
summary(modelo5)
...
## ---------------------------------------
##Etapa 7 - FAzendo previsões do modelo de treino

#Probabilidade da pessoa desenvolver ou não doença hepática
previsoes_treino <- predict(modelo5, dados_treino_balanceado,type = "response")

resultados_treino <- data.frame(previsoes_treino, dados_treino_balanceado$DOENCA_HEPATICA)
...
## ---------------------------------------
##Etapa 8- Desempenho do modelo de treino


# Matriz de confusão e medidas
library(caret)
matriz_confusao_treino = confusionMatrix(resultados_treino$previsto_cat, dados_treino_balanceado$DOENCA_HEPATICA, positive = "1")
matriz_confusao_treino

#Curva ROC
library(ROSE)
roc.curve(dados_treino_balanceado$DOENCA_HEPATICA, previsoes_treino, plotit = T, col = "red")
